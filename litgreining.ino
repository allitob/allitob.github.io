...
void loop(){
  ...
  //float TOT = abs(Red) + abs(Green) + abs(Blue);
  //float Mean = TOT/3;
  ...
  if (Red <=30 && Green <=35 && Blue <=38)         //If the values are low it's likely the white color (all the colors are present)
      Serial.println("\n \n \n Aspassúpa");                    
      
  else if (Red<Blue && Red-Green <= 3) //&& Red<40)      //if Red value is the lowest one and smaller thant 23 it's likely Red
      Serial.println("\n \n \n Tómatssúpa");

  else if (Blue<Green && Blue<Red && Blue<40 || abs(Red - Green) < 5 && abs(Red - Blue) < 5)    //Same thing for Blue
      Serial.println("\n \n \n Aspassúpa");

  //else if (Green-Blue<= 12)           //Green it was a little tricky, you can do it using the same method as above (the lowest), but here I used a reflective object
     ...
     Serial.println("\n \n \n Aspassúpa");                  //if the color is not recognized, you can add as many as you want
